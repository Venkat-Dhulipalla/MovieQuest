  ...
  return (store) => (next) => async (action) => {
    ...
    if (!meta.handlers || !Object.keys(meta.handlers).length)
      return console.error(
        `action must contain non empty 'meta' object with at least one handler`,
        action
      );
    ...
    !task.isDummy &&
      task.subscribe(ActionTask.events.CANCEL, function () {
        cancelToken.cancel();
        if (debug) log(`Action has been canceled`);
        if (handlers.cancel)
          dispatchAction(handlers.cancel, null, store.dispatch);
      });
    ...
    try {
      ...
      if (method.toLowerCase() === "get")
        response = await get(url, params, cancelToken);
      // else if () { ... }
      else {
