...
function Movie(props) {
  ...
  const { isAppReady, movie, fetchMovie, toggleFavorite, makeMovieVisited } =
    props;
  ...
  useEffect(
    ...
    [movie.id, movie.isFetched, makeMovieVisited]
  );
...
function MovieInfo({ movie, loaded, onFavorite }) {
  ...
  const poster =
    posterImageUrl || require("../assets/images/abstract_movie_poster.svg");
          ...
          <Grid container spacing={7}>
            ...
            <Grid item md={8} style={{ color: "white" }}>
              ...
              <ul className={classes.genreList}>
                {genres.map((genre) => (
                  <li className={classes.genre} key={genre}></li>
                ))}
              </ul>
...
function MovieRecommendations(props) {
  ...
  const recommendedMovies = recommended.ids
    .slice(0, MOVIES_PER_LIST)
    .map((id) => getMovie(id, entities, user));
  const similarMovies = similar.ids
    .slice(0, MOVIES_PER_LIST)
    .map((id) => getMovie(id, entities, user));

  const fetchSimilarMoviesCb = useCallback(
    () => fetchSimilarMovies(id),
    [id, fetchSimilarMovies]
  );
  const fetchRecommendedMoviesCb = useCallback(
    () => fetchRecommendedMovies(id),
    [id, fetchRecommendedMovies]
  );
...
function MovieList({
  ...
  if (isFetched && !movies.length)
    return <EmptyBlock text={"There is no data for this movie"} />;
...
function mapStateToProps(state) {
  let movie = state.movie.isFetched
    ? getMovie(state.movie.id, state.entities, state.user)
    : {};
